...
var decimal = require('../../decimal.js-light/decimal.cjs');
var utils = require('./util/utils.cjs');
var arithmetic = require('./util/arithmetic.cjs');
...
function getFormatStep(roughStep, allowDecimals, correctionFactor) {
  ...
  var digitCount = arithmetic.default.getDigitCount(roughStep.toNumber()); // The ratio between the rough step and the smallest number which has a bigger
...
function getTickOfSingleValue(value, tickCount, allowDecimals) {
  ...
  if (!middle.isint() && allowDecimals) {
    ...
    if (absVal < 1) {
      ...
      step = new decimal.default(10).pow(arithmetic.default.getDigitCount(value) - 1);
...
function getNiceTickValuesFn(_ref3) {
  ...
  var values = arithmetic.default.rangeStep(tickMin, tickMax.add(new decimal.default(0.1).mul(step)), step);
...
function getTickValuesFixedDomainFn(_ref7, tickCount) {
  ...
  var values = [].concat(_toConsumableArray(arithmetic.default.rangeStep(new decimal.default(cormin), new decimal.default(cormax).sub(new decimal.default(0.99).mul(step)), step)), [cormax]);
  return min > max ? utils.reverse(values) : values;
}
