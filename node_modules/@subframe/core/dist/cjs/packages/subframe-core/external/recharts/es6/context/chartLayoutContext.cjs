...
var find = require('../../../../../../external/lodash/find.cjs');
var every = require('../../../../../../external/lodash/every.cjs');
var calculateViewBox = require('../util/calculateViewBox.cjs');
...
var useXAxisOrThrow = function useXAxisOrThrow(xAxisId) {
  ...
  !(xAxisMap != null) ? process.env.NODE_ENV !== "production" ? tinyInvariant.default(false, 'Could not find Recharts context; are you sure this is rendered inside a Recharts wrapper component?') : tinyInvariant.default(false) : void 0;
  var xAxis = xAxisMap[xAxisId];
  !(xAxis != null) ? process.env.NODE_ENV !== "production" ? tinyInvariant.default(false, "Could not find xAxis by id \"".concat(xAxisId, "\" [").concat(_typeof(xAxisId), "]. ").concat(getKeysForDebug(xAxisMap))) : tinyInvariant.default(false) : void 0;
  return xAxis;
};
...
var useYAxisWithFiniteDomainOrRandom = function useYAxisWithFiniteDomainOrRandom() {
  ...
  var yAxisWithFiniteDomain = find.default(yAxisMap, function (axis) {
    return every.default(axis.domain, Number.isFinite);
  });
...
var useYAxisOrThrow = function useYAxisOrThrow(yAxisId) {
  var yAxisMap = React.useContext(YAxisContext);
  !(yAxisMap != null) ? process.env.NODE_ENV !== "production" ? tinyInvariant.default(false, 'Could not find Recharts context; are you sure this is rendered inside a Recharts wrapper component?') : tinyInvariant.default(false) : void 0;
  var yAxis = yAxisMap[yAxisId];
  !(yAxis != null) ? process.env.NODE_ENV !== "production" ? tinyInvariant.default(false, "Could not find yAxis by id \"".concat(yAxisId, "\" [").concat(_typeof(yAxisId), "]. ").concat(getKeysForDebug(yAxisMap))) : tinyInvariant.default(false) : void 0;
  return yAxis;
};
